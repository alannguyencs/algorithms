...
void lowwer_envelope(VPT &pts, VT &x){
    ...
    VPT dn;
    if(pts.size() > 1) x.push_back(xIntersect(pts[0], pts[1])), dn.push_back(pts[0]), dn.push_back(pts[1]);
    For(i,2,pts.size() - 1){
        while(dn.size() > 1 && xIntersect(pts[i], dn.back()) <= x.back()) dn.pop_back(), x.pop_back();
        x.push_back(xIntersect(pts[i], dn.back())); dn.push_back(pts[i]);
    }
    pts = dn;
}
