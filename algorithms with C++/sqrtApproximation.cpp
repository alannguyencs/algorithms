...
BigInt::BigInt( long double d )
{
    ...
    while( d > 0 )
    {
        digits[size++] = ( int )( ( d - floor( d / 10 ) * 10 ) + 0.5 );
        d = floor( d / 10 );
    }
}
...
int main(){
	...
	std::ios::sync_with_stdio(false);
        ...
        x^2 – D*y^2 = 1  --> x = num; y = dem;
        int D1 = sqrt(D);
        BigInt D(i), limit(D1);
        if (limit * limit == D) continue; //xet rieng truong họp nay
        BigInt m(0), d(1), a = limit, numm1(1), num = a, denm1 (0), den (1);
        while(!(num*num - D*den*den == BigInt(1))){
            m = d * a - m; d = (D - m * m) / d; a = (limit + m) / d;
            BigInt numm2 = numm1; numm1 = num; BigInt denm2 = denm1; denm1 = den;
