...
/*
This is to find number of distinct substring of a string S
example: aaaaaa, abcbca
*/
ll a[10000000];
int cnt;
int sol(string s){
    ...
    for(int i = 0; s[i]; i++){
        ll h = 0ll;
        for(ll j = i; s[j]; j++){ // bat dau dem so substring bat dau tu j = i
            h = (h*131)^s[j];        //I don't know why 131
            a[cnt++] = h;
        }
    }

    sort(a, a + cnt);
    cout << unique(a, a + cnt) - a << el;
